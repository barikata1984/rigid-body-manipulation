...
- 2026-08-26 関節位置ノイズの標準偏差を並進 2.0e-5 m, 回転 1.0e-4 rad とする / 却下: 純粋な読み取りノイズとして 1e-5 rad を採る — エージェント判断. 後続の実機ログ分析により、 `empirical` profile では回転 `1.5e-5 rad`、並進 `2.0e-5 m` へ置換した。並進値はさらに 2026-08-27 の決定で `1.0e-5 m` へ変更した ([議事録](LOGS/2026-08-26_dataset-merge-and-noise-model.md))
...
- 2026-08-26 速度・加速度のノイズ係数を実効微分窓幅 32.6ms に基づく値 (速度 43, 加速度は fps の 1 乗に比例し 60fps で 1840) とする / 却下: 現行の位置 2 回差分モデル (速度 √2·fps, 加速度 √6·fps²) を維持する — ユーザー確認済み. 後続の `empirical` profile では 34 ms 因果差分速度とゲイン 10 の一次 LPF 加速度へ置換した ([議事録](LOGS/2026-08-26_dataset-merge-and-noise-model.md))
...
- 2026-08-27 `empirical` 系 profile の並進位置ノイズを `1.0e-5 m` とする。これは UR5e 手先観測等価値 `[7.289e-6, 5.131e-6, 8.398e-6] m` の保守的丸めであり、物理直動軸の校正値ではない / 却下: `2.0e-5 m` を名目値として維持 — ユーザー確認済み ([状態更新](LOGS/2026-08-26_dataset-merge-and-noise-model.md#2026-08-27-status-update))
...
- 2026-08-28 慣性同定用の励起軌道生成には D-opt を採用し、現行成果物は seed44 を使う / 却下: 列等化条件数目的 — エージェント判断 ([議事録](LOGS/2026-08-28_inertial-identification-data-collection-rationale.md))
- 2026-08-28 raw 条件数目的 seed42 を次点として保持する — エージェント判断 ([議事録](LOGS/2026-08-28_inertial-identification-data-collection-rationale.md))
