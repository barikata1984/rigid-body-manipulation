...
> **後続セッションによる更新 (2026-07-15)**: 本記録の cond 比較は早期停止 (`target_condition_number=10`) による打ち切りを含んでおり、悪化倍率を過小評価していた。また機構として提案した「スペクトル衝突」は、後続セッションの対照ペア実験 (base ピーク角加速度一致、スペクトル帯 2 倍差で cond 一致) により棄却された。収束実験と固有値分解に基づく更新後の機構 (最小固有値 ixx+iyy 方向の崩壊) は `20260715T021417Z_task_drift_excitation_convergence_mechanism.md` を参照。本記録の「スペクトル衝突」「相対振幅比を核心とする機構」の記述は上書きされている。
