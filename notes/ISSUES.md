...
## [2026-08-27] 第5関節 pitch の ±45°制約に制御安定性の根拠がない

既存設定は球面手首の運動学的特異点 pitch=±π/2 に対する安全余裕として、pitch を ±π/4 に制限していた。
しかし、現在の制御器は task Jacobian を反転しない関節空間 LQR である。
ユーザー環境の正式試験では pitch を 0→−π→+π と動かし、単独試験と他の5関節を同時に動かす試験がともに発散せず完了した。
計測付き単独試験の最大追従誤差は 0.00406 rad だった。
...
目標列は `int(duration * fps)` で切り捨てて生成される一方、`Simulator.run()` の記録判定は境界 frame を要求する。
この不一致により、軌道長が frame 境界に一致しない場合は目標列を使い切った後に `IndexError` となる。
今回の正式試験では軌道長を 60 Hz の境界へ合わせて回避した。
実装修正と境界条件の回帰試験は未着手である。
詳細は `notes/LOGS/2026-08-27_pitch-joint-range-control-validation.md` を参照する。
