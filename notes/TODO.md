...
- [ ] センサーノイズ (`sensors/sensors.py:17-23` の jointpos_stddev / jointvar_noise_scaler) と同定パイプラインの整合をとる設計判断: (a) `SimulatorConfig` に `perturbation: bool` フラグを追加してユーザーが実験毎に切り替え, (b) ノイズなしを default にして「実機模擬モード」だけ明示的に on, (c) MuJoCo 真値 (`d.qpos/qvel/qacc`) をメタデータ保存して事後にノイズ再合成可能にする, (d) EIV (errors-in-variables) を明示的に扱う回帰式に変更する, のどの組み合わせを取るか. FTA 結果: ノイズを切れば TLS L2=0.000173 でほぼ完全同定可能. 詳細: notes/LOGS/log_trajectory_optimization.md 2026-07-12 (続 2) エントリ, notes/ISSUES.md
  ...
  - [x] wrench (force/torque) センサーに独立スケールのノイズを追加し, 回帰行列 Y とターゲット τ の両方にノイズが乗る errors-in-variables 問題として LS/TLS を比較する (2026-07-13: `force_noise_scale`/`torque_noise_scale`/`perturb_wrench` を追加し実装. EIV スイープ (kinematics noise=0.1 固定, wrench scale W 可変) の結果, 事前仮説「wrench ノイズで TLS が LS に勝つ」は棄却された. LS は wrench ノイズにほぼ不感 (バイアス源は回帰子 Y のノイズであって τ 単独のノイズではない), 素の TLS は W=0.5 から劣化し W=2 で崩壊 (L2=0.449). 素の TLS (`total_lstsq` = 重みなし `scipy.odr`) は [Y|τ] 全列の等分散を仮定するため, wrench 列だけノイズが大きい heteroscedastic 状況で過補正する. 詳細: notes/LOGS/log_trajectory_optimization.md 2026-07-13 エントリ)
  - [ ] `jointpos_stddev` / `force_stddev` / `torque_stddev` を実機の関節エンコーダおよび FT センサー仕様に基づいてグラウンディングする (現状は未検証の見積もり. 並進 prismatic (5e-4 m) と回転 revolute (1e-3 rad) は単位が異なるため分けて評価する. マニピュレータ `xml_models/manipulators/sequential` が実在機の模擬か汎用仮想機かの素性確認も含む)
  - [ ] wrench EIV スイープで判明した素の TLS の heteroscedastic 破綻への対処: 実機調査で FT ノイズが運動学ノイズに対し不均衡に大きいと分かった場合に限り, 列ごとの既知 σ を重みとして `scipy.odr` に渡す scaled/generalized TLS への変更を検討する
