...
- [x] q4(pitch) の特異点回避が構造的に機能しない問題を解消する (2026-07-08: singularity_center/margin による q4=0 近傍の追加除外を無効化し, 当時は q_min/q_max の ±45° 制限を維持. 2026-08-27: ユーザー環境の関節空間 LQR で pitch を ±180°まで動かし、単独・多関節とも ±90°通過時を含め発散しないことを確認. ±45°を制御安定性要件として維持する判断は撤回. → notes/LOGS/2026-08-27_pitch-joint-range-control-validation.md)
- [ ] 第5関節 pitch の q_min/q_max を実機可動範囲に置き換え、設定に残る古い「pitch=0 で LQR が発散する」コメントを削除する (モデル上は ±180°まで追従確認済み. → notes/LOGS/2026-08-27_pitch-joint-range-control-validation.md)
- [ ] 解析的フーリエバウンドを使わない直接 q/dq/ddq 制約の最適化済み励起軌道で、拡張した pitch 範囲の制約充足と追従安定性を確認する
...
- [ ] `duration * fps` が整数でない場合に目標 frame が先に尽きて `Simulator.run()` が `IndexError` になる問題を修正し、境界条件を回帰試験化する (→ notes/LOGS/2026-08-27_pitch-joint-range-control-validation.md)
...
- [ ] センサーノイズ (`sensors/sensors.py:17-23` の jointpos_stddev / jointvar_noise_scaler) と同定パイプラインの整合をとる設計判断: (a) `SimulatorConfig` に `perturbation: bool` フラグを追加してユーザーが実験毎に切り替え, (b) ノイズなしを default にして「実機模擬モード」だけ明示的に on, (c) MuJoCo 真値 (`d.qpos/qvel/qacc`) をメタデータ保存して事後にノイズ再合成可能にする, (d) EIV (errors-in-variables) を明示的に扱う回帰式に変更する, のどの組み合わせを取るか. FTA 結果: ノイズを切れば TLS L2=0.000173 でほぼ完全同定可能. 詳細: notes/LOGS/log_trajectory_optimization.md 2026-07-12 (続 2) エントリ, notes/ISSUES.md
  ...
  - [x] 慣性パラメータ 10 個のパラメータ別誤差分析を実施する (done 2026-08-26: GT・推定値・絶対誤差・誤差率を全成分で算出. 今後は質量・重心・慣性対角を主評価, 慣性非対角を副評価, L2 を診断指標とする. → notes/LOGS/2026-08-26_dataset-merge-and-noise-model.md)
