...
- **減衰バイアス**: 回帰行列 A は関節観測 (qpos/qvel/qacc) の関数で、観測ノイズが
  ...
  −50% 級の縮みが残る。
  これは現行 `empirical` profile より前の経過値である。2026-08-27 の同期後評価は並進 `2e-5 m`、回転 `1.5e-5 rad` で実施した。その後、現行 profile の並進値を UR5e 手先観測等価値の保守的丸めである `1e-5 m` へ変更した。速度・加速度は同一位置系列から生成する。同期後評価は `notes/debug/2026-08-26_merged-dataset-and-ols-results.md` 4.7 節を参照する。
